...
public class PlottingFloats {
	...
	float PAD = 25;
	java.util.List<Float> dataList = new ArrayList<Float>();
	...
	public void paintComponent(Graphics g) {
		...
		float max = Collections.max(dataList).floatValue();
		@SuppressWarnings("unchecked")
		float min = Collections.min(dataList).floatValue();
		...
		for(int j = 0; j < dataList.size(); j++) {
			y = yOrigin - scale * (dataList.get(j).floatValue() - yDataOffset);
...
public static void main(String[] args) {
	float[] data = {100f, 230f, 12.9f, 65f, 47.3f, 175f, 190f, 18f, 255f};
