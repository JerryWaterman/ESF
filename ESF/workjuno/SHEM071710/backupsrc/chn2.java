	   	...
	   	for(int i =0; i < temp.length ; i++) {
	   		...
	   		if ((v18 == 0) && (vnext == 18)){ 
	   			...
	   			if (vtemp.length() == 0){
	   				...
	   				System.out.println(" v length = "+vlen);
	   				String vstr = vtemp.substring(4,7);
	   				int tindex = vtemp.indexOf("apr", 0);
	   				if (tindex > 0){
	   					if (vtemp.substring(tindex, tindex+3).equals("apr")) {
	   						vs18 = "'04-',";
	   						//System.out.println("     inside if  'apr',  vs18="+vs18);
	   					}                       //	System.out.println("tindex = "+ tindex);
	   				}       	   				//  vs18 = vtemp + ",";
	   				System.out.println("vtemp = "+vtemp+ " vstr = "+ vstr);  
	   				//vtemp = 9-10apr07    to   '2007-04-09','2007-04-10'
	   				ArrayList<Integer> myArr = new ArrayList<Integer>();
	   				for (int i1 = 0; i1 < vlen; ++i1 ) {
	  					char oneChar = vtemp.charAt(i1);
	   					boolean retval = Character.isDigit(oneChar);
	   					if (retval){
	   						int numi = Integer.valueOf(oneChar).intValue();
	   						myArr.add(numi);
	   					    System.out.println(" inside for loop is digit "+i1+"  Char = "+oneChar);
	   					}else{
	   					    System.out.println("is not digit"+i1);
	   					}
	   				}
	   				for (Iterator<Integer> iter = myArr.iterator(); iter.hasNext();  ) {
