...
public class dataInternalFrame implements ActionListener {
	...
	*/	
	 ...
	 * @param headers
	 */
	public dataInternalFrame(String[] headers){
		...
		frame.setLocation(dim.width / 2 - frame.getWidth() / 2, dim.height / 2
 	    ...
 	    frame.setTitle("Set up the data for the Periphyton table in RTP. Ver. 0.2 Jan. 31, 2011");
 	    frame.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
 	    frame.setAlwaysOnTop(true);
        Container content = frame.getContentPane();     //702-1052 Scott
	...
	private void formatCells() {
		...
		int numberofColumns = inputTable.getModel().getColumnCount()-1; // numberofColumns=39
		
		for(int k = 1; k < numberofColumns; k++){// was 26 now numberofColumns
			column1 = colMod.getColumn(k);
			column1.setPreferredWidth(55);//40
		...
		while (columns.hasMoreElements()) {
			 ...
			 //System.out.println("column  "+columns.nextElement()+"    k = "+k);
